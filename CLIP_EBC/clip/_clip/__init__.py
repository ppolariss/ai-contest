...
def load_partial_state_dict(model, state_dict, skip_layers):
    own_state = model.state_dict()
    for name, param in state_dict.items():
        if name.split('.')[0] in skip_layers:
            print(f"Skipping loading parameter: {name}")
            continue
        if name in own_state:
            own_state[name].copy_(param)
        else:
            print(f"Ignoring parameter: {name}")
...
def _resnet(
    ...
    **kwargs: Any
    ...
    skip_layers = ["conv1"]
    load_partial_state_dict(model, state_dict, skip_layers)
